...
class GRN(nn.Module):
    ...
    def forward(self, nodes, neighbors, attention_scores):
        ...
        for i in range(len(neighbors)):
            #print(neighbors[i].shape)
            individual_neighbors = torch.split(neighbors[i], 1, dim=0)
            #print(len(individual_neighbors))
            out_nodes = torch.zeros((1,self.num_of_heads, self.num_out_features), dtype=in_nodes_features.dtype, device=self.device)
            for j in range(len(individual_neighbors)):
                #print(individual_neighbors[j].shape)
